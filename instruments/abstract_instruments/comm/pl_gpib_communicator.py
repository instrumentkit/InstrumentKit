...
# -*- coding: utf-8 -*-
"""
Provides a communication layer for an instrument connected via a Prologix
GPIB adapter.
"""
...
class PLGPIBCommunicator(io.IOBase, AbstractCommunicator):

    """
    Communicates with a SocketCommunicator or SerialCommunicator object for
    use with Prologix GPIBUSB or GPIBETHERNET adapters.

    It essentially wraps those physical communication layers with the extra
    overhead required by the Prologix GPIB adapters.
    """

    # pylint: disable=too-many-instance-attributes
    def __init__(self, filelike, gpib_address):
        super(PLGPIBCommunicator, self).__init__(self)

        self._file = filelike
        self._gpib_address = gpib_address
        self._file.terminator = "\r"
        self._version = float(self._file.query("++ver").split(' ')[-1])
    ...
    def _sendcmd(self, msg):
        ...
        the Prologix GPIB adapter. This function is in turn wrapped by
